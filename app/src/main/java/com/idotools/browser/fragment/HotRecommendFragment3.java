...
public class HotRecommendFragment3 extends BaseFragment implements View.OnClickListener {
    ...
    private synchronized void bindViewData() {
        LogUtils.e("绑定数据，长度是:" + mDmzjList.size());
    ...
    private void dmzjBeanBindView(DmzjBeanResp.DmzjBean bean, ImageView imageView, TextView textView) {
        if (bean != null) {
            if (mContext != null) {
                GlideUtils.loadImage(mContext, bean.cover, imageView);
            }
            LogUtils.e("textview="+textView);
